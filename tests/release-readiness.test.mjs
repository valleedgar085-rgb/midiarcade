...
test("Android release configuration targets the current Play baseline without broad media access", async () => {
  ...
  assert.match(appGradle, /versionName\s+"1\.2\.1"/);
...
test("Android APK workflow verifies, syncs, builds, and preserves the installable artifact", async () => {
  ...
  assert.match(workflow, /name: midi-arcade-1\.2\.1-preview-apk/);
...
test("PWA and Play artwork is versioned, present, and wired into the studio product", async () => {
  ...
  assert.match(html, /class="version-chip">V1\.2\.1</, "the visible release badge must match the package version");
