...
for (const genre of GENRES) {
  for (const seed of SEEDS) {
    ...
    rows.push({
      ...
      densityGapBefore: round(finite(beforeEval.diagnostics?.densityTarget) - pitchedNotesPerBar(before)),
      densityGapAfter: round(finite(afterEval.diagnostics?.densityTarget) - pitchedNotesPerBar(after)),
